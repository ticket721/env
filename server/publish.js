    ...
    switch (branch) {
        case 'rinkeby': {
            ...
            try {
                ...
                execSync(`docker tag server-modules ${process.env.DOCKER_SERVER_MODULES_REPOSITORY}:latest-rinkeby`);
                execSync(`docker tag server ${process.env.DOCKER_SERVER_REPOSITORY}:${tag_name}`);
                execSync(`docker tag server ${process.env.DOCKER_SERVER_REPOSITORY}:latest-rinkeby`);
                ...
                execSync(`docker push ${process.env.DOCKER_SERVER_MODULES_REPOSITORY}:latest-rinkeby`);
                execSync(`docker push ${process.env.DOCKER_SERVER_REPOSITORY}:${tag_name}`);
                execSync(`docker push ${process.env.DOCKER_SERVER_REPOSITORY}:latest-rinkeby`);
