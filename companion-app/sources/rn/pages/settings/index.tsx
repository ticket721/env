...
import { ResetCompanion }                  from '../../../redux/actions/device';
...
export interface SettingsPageRDispatch {
    dispatch: Dispatch;
}

export type SettingsPageMergedProps = SettingsPageProps & ThemedComponentProps & SettingsPageRDispatch;

const settingsItems = [
    {
        title: 'settings_reset_companion',
        logo: require('../../../assets/reset.png'),
        action: ResetCompanion
    }
];
...
class SettingsPage extends React.Component<SettingsPageMergedProps> {
            ...
            >
                <View style={{flex: 1, alignItems: 'center', justifyContent: 'flex-start', flexDirection: 'row'}}>
                    <Image
                        ...
                        height={30}
                    />
                    <Text style={{fontSize: 18, lineHeight: 18, marginLeft: 16}}>{I18N.t('settings_reset_companion')}</Text>
                </View>
            </ListItem>
        )
...
const mapDispatchToProps = (dispatch: Dispatch): SettingsPageRDispatch => ({
    dispatch
});
